...
module.exports.getAsAdmin = async (req, res) => {
    try {
        ...
        const result = await db.sequelize.query(`SELECT ud.id, ud.amount, ud.created_at, c.name as creator, d.name as drink, cat.name as category, d.price as price, ud.amount * d.price as sumprice FROM user_drinks AS ud INNER JOIN users as u ON ud.user_id = u.id INNER JOIN users as c ON ud.creator_id = c.id INNER JOIN drinks as d ON ud.drink_id = d.id LEFT JOIN categories as cat ON d.category_id = cat.id WHERE ud.user_id = ${ id } ORDER BY ud.created_at DESC LIMIT ${ stepsize } OFFSET ${ offset };`, { type: QueryTypes.SELECT });
...
module.exports.getEntriesAsAdmin = async (req, res) => {
    try {
        ...
        const result = await db.sequelize.query(`SELECT COUNT(*) FROM user_drinks WHERE user_id = ${ id };`, { type: QueryTypes.SELECT });
        numEntries = parseInt(result[0].count);
...
module.exports.add = async (req, res) => {
    try {
        if (!req.body.amount || !req.body.drinkId) {
        ...
        const updatedMoney = await db.sequelize.transaction(async t => {
            ...
            const user = await db.User.findOne({ where: { id: req.userId }}, { transaction: t });
...
module.exports.addAsAdmin = async (req, res) => {
    try {
        if (!req.body.amount || !req.body.drinkId) {
            throw new Error('Request not complete');
        }
        ...
        const updatedMoney = await db.sequelize.transaction(async t => {
            ...
            const user = await db.User.findOne({ where: { id: req.params.id }}, { transaction: t });
            if (!user) throw new Error('User not found');
