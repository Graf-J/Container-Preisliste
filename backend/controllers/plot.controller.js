...
module.exports.getOwnCategories = async (req, res) => {
    try {
        const categories = await db.sequelize.query(`SELECT COALESCE(c.name, 'Other'), SUM(ud.amount) AS total FROM user_drinks AS ud INNER JOIN drinks AS d ON ud.drink_id = d.id LEFT JOIN categories AS c ON d.category_id = c.id WHERE ud.user_id = ${ req.userId } GROUP BY c.id ORDER BY total DESC;`, { type: QueryTypes.SELECT });
