...
import { getEntriesCountAsAdmin, getUserCredits, deleteCredit, addCredit } from '../../services/creditService';
import './CreditTab.css';

const CreditTab = ({ getTabHeight, user, setUser }) => {
    ...
    const fetchCredits = async () => {
        ...
        const credits = await getUserCredits(userId, stepsize, page);
    ...
    const handleCreateCreditClick = async () => {
        if (money < 0.01) {
            ...
            const updatedMoney = await addCredit({ money: money, userId: userId });
            setUser({ name: user.name, money: updatedMoney });
                ...
                <div className='credit-list'>
                    { credits.map(credit => (
                        <div key={ credit.id } className='credit-entry'>
                            <div className='credit-entry-delete-wrapper'>
